...
function get_meta_imagesampler( $args = array(), $value = false ){ 
	...
	<td>
						...
						if(typeof jQuery.wp === "object" && typeof jQuery.wp.wpColorPicker === "function"){
							var options = {
								...
								clear: function(event, ui) {
									jQuery('#thumbnail_hover_color').val('');
								},
			...
			if(is_array($parsed_url) && is_array($parsed_site_url)){
				...
				if(($host == $site_host) && ($protocol == $site_protocol)){
					...
					//if(strlen(end(explode('.', $parsed_url['path']))) >= 1 && strlen(end(explode('.', $parsed_url['path']))) <= 5){
						$image_path = get_template_directory_uri() . '/admin/superslide/flow_get_image.php?image='.esc_html($value);
					//}
				}
			}
		?>
		<script type="text/javascript">
			...
			var site_image_getter = '<?php echo get_template_directory_uri() . '/admin/superslide/flow_get_image.php?image='; ?>';
